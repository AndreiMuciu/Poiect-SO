...
int identific_fisiere_periculoase(const char *fileName) {
    ...
    if (stat(fileName, &st) == -1) {
        perror("Eroare la verificarea permisiunilor fișierului\n");
        exit(EXIT_FAILURE);
    }
...
int listFiles(const char *basePath, const char *output, const char *izolare, int undeSunt) {
    metadate prev;
    metadate actual;
    char path[2000];
    char buffer[2048];
    ...
    if(adv == 1) {
        ...
        if (fd == -1) {
            perror("Eroare la re-deschiderea fișierului pentru scriere");
            exit(EXIT_FAILURE);
        }
...
void verific_Linia_de_comanda(int argc, char *argv[]) {
    struct stat statbuf;
    int directory_count = 0, numarPanaLa_o = 1, numarPanaLa_x = 1, val, imp = 0;
    char output[32], individualOutput[32], izolare[32];
    pid_t pid;
    ...
    for (int i = 1; i < numarPanaLa_o; i++) {
        if (stat(argv[i], &statbuf) == 0) {
            if (S_ISDIR(statbuf.st_mode)) {  // Verificăm dacă este director
                directory_count++;  
    ...
    if(imp == 1) {
        ...
        for (int i = 1; i < numarPanaLa_o; i++) {
            if (stat(argv[i], &statbuf) == 0) {
                if (S_ISDIR(statbuf.st_mode)) {  // Verificăm dacă este director 
                    actualizareOutputGeneral(argv[i], output);
                }
            } else {
                printf("Eroare: Nu se poate accesa '%s'. Verifica existenta si permisiunile.\n", argv[i]);
                exit(EXIT_FAILURE);
